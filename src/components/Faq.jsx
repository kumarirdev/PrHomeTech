...
function Faq() {
  ...
  return (
    <>
      ...
      <section className="main-container faq-section px-15">
        <div className="faq-container flex gap-10 items-start justify-between w-full mt-10">
          <div className="faq-intro w-[50%] mt-15">
          ...
          <div className="grid gap-4 w-[50%] ">
            {questions.map((item, index) => {
              ...
              return (
                <div
                  className={`overflow-hidden rounded-[18px]  border border-[#e5e3eb] bg-[#f8f7fb] ${
                    isOpen ? "bg-[#f5f3fa]" : ""
                  }`}
                  key={item.question}
                >
                  <button
                    className="flex min-h-[68px] w-full cursor-pointer items-center justify-between border-0 bg-transparent px-5 py-[18px] text-left text-[17px] font-semibold text-[#18171e]"
                    type="button"
                  ...
                  >
                    ...
                    <div className="bg-[#0236A4] text-white w-8 rounded-full flex items-center justify-center  h-8">{isOpen ? <Minus size={16}/> : <Plus size={16}/>}</div>
                    
                  </button>
                  {isOpen && (
                    <p
                      className="mx-5 mb-[22px] mt-[-2px] max-w-[650px] text-sm leading-[1.55] text-[#77757f]"
                      id={`faq-answer-${index}`}
                    >
                      {item.answer}
                    </p>
